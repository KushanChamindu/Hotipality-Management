package com.mbpackageservice.repository.specifications;
...
public class ActivitySpecifications {
        public static Specification<Activity> filter(Double latitude, Double longitude, Double distance,
                        EventType type, EventLevel level, Float review, Boolean availability, Double lowerPrice,
                        Double upperPrice) {
                return (root, query, criteriaBuilder) -> {
                        ...
                        if (longitude != null && latitude != null && distance != null) {
                                nearestPackages = findActivitiesWithinDistance(root, query, criteriaBuilder, latitude,
                                                longitude, distance);

                        } else {
                                nearestPackages = criteriaBuilder.conjunction();
                        }
                        return criteriaBuilder.and(packageType, packageLevel, packageReview, nearestPackages,
                                        packageAvailability, packagePrice);
