package com.example.megablissmainservice.Controller;
...
public class UserController {
    ...
    public List<UserStatistic> getUserStatisticByDate(
            ...
            @Past(message = "Start date must be in the past or today")
            LocalDate startDate,
            @RequestParam(name = "endDate", required = false)
            @Past(message = "End date must be in the past or today")
            LocalDate endDate){
        List<UserStatistic> userStatisticList = uis.getUserStatisticByDate(startDate, endDate);
        return userStatisticList;
    }

//    only manually used when missing historical user statistic data before yesterday
